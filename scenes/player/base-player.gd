...
@onready var states_manager: StatesManager = $StatesManager
...
var actions: Array[Action] = [
	Action.new('base_attack', PlayerBaseState.State.BaseAttack),
	Action.new('ghost_attack', PlayerBaseState.State.FastAttack, 10),
	Action.new('ki_blast', PlayerBaseState.State.KiBlast, 2),
	Action.new('teleport', PlayerBaseState.State.Teleport, 20),
	Action.new('guard', PlayerBaseState.State.Defense)
]
...
func init(character_name: String, character_data: CharacterDTO):
	self.character_name = character_name
	self.sprite_name = character_data.sprite
	self.stats = character_data.stats
	self.sprite = Utility.load_sprite(self.character_name + '/' + self.sprite_name)
	add_child(self.sprite)
	
func _ready():
	var character_states := CharactersStates.get_states(self.character_name)
	self.states_manager.load_character_states(self, self.character_name, character_states)
	
	self.actions.resize(5)
	for node in character_states:
		actions.append(node.action)
		
	self.machine_state = self.states_manager
	self.machine_state.init(self)
	self.set_zoom_camera()
	Events.connect('update_zoom_camera', self.set_zoom_camera)
	#Events.connect('hit', hit)
	#Events.connect('level_up_animation', level_up_animation)
	Events.connect('get_item', self.get_item)
	Events.connect('send_experience_to_player', self.update_experience)
	Events.connect('item_used', self.item_used)
	Events.connect('grab_health_drop', self.grab_health_drop)
	Events.connect('grab_energy_drop', self.grab_energy_drop)
	#machine_state.connect('state_changed', state_timer, 'state_changed') 
	for timer in [self.state_timer, self.energy_regen_timer, self.defense_regeneration_timer]:
		self.add_child(timer)
	self.energy_regen_timer.wait_time = 5
	self.energy_regen_timer.one_shot = true
	self.defense_regeneration_timer.wait_time = 3
	self.defense_regeneration_timer.one_shot = true
...
func end_generic_change_map(fly: bool = false) -> void:
	var state := PlayerBaseState.State.FlyChangeMap if fly else PlayerBaseState.State.ChangeMap
	machine_state.states[state].complete()
	set_physics_process(false)
...
func play_attack_animation(animation: String, backward: bool = false) -> void:
	...
	animation =  animation + '_' + str(random_animation) + '_' + Direction.get_direction_animation(direction, exists_left_animation)
	sprite.play(animation, backward)
	
#PROPERTY REGION
func set_direction(new_direction: int) -> void:
	var changed = _direction != new_direction
	if changed:
		_direction = new_direction
		#set_hitwall()
		play_animation(machine_state.current_state.animation)
		base_attack_area.set_rotate(_direction)

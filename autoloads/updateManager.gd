...
func _ready() -> void:
	if not FilesSystemManager.updates_file_exists(): FilesSystemManager.create_update_file()
		
	var data = FilesSystemManager.load_updates_file()
	...
	for i in range(settings.get('version'), data.keys().size()):
		...
		for area in current_data.keys():
			var area_data = FilesSystemManager.load_save_file(Paths.MAPS + area)
			if area_data:
			...
			FilesSystemManager.update_save_file(Paths.MAPS + area, area_data)
		
	FilesSystemManager.update_settings_file(settings)
